...
const validateClientAction = (req, res, next) => {
    const { id } = req.params;
...
router.get('/admin/clients', auth_1.adminAuthMiddleware, async (req, res) => {
    try {
    ...
    `;
        ...
        const clients = await database_1.default.query(query, params);
        res.json(clients);
    }
    catch (error) {
...
router.get('/admin/clients/:id', auth_1.adminAuthMiddleware, validateClientAction, async (req, res) => {
    try {
        const { id } = req.params;
        // Get client basic info
        const clients = await database_1.default.query('SELECT * FROM clients WHERE id = ?', [id]);
        if (clients.length === 0) {
...
router.post('/admin/clients/:id/toggle-status', auth_1.adminAuthMiddleware, validateClientAction, async (req, res) => {
    try {
        const { id } = req.params;
...
router.delete('/admin/clients/:id', auth_1.adminAuthMiddleware, validateClientAction, async (req, res) => {
    try {
        const { id } = req.params;
        const { notes } = req.body;
        const adminId = req.admin.id;
...
router.get('/admin/clients/stats/overview', auth_1.adminAuthMiddleware, async (req, res) => {
    try {
        const [totalClients, activeClients, suspendedClients, clientsThisMonth, topClients] = await Promise.all([
            database_1.default.query('SELECT COUNT(*) as count FROM clients WHERE status != "deleted"'),
            database_1.default.query('SELECT COUNT(*) as count FROM clients WHERE status = "active"'),
            database_1.default.query('SELECT COUNT(*) as count FROM clients WHERE status = "suspended"'),
            database_1.default.query('SELECT COUNT(*) as count FROM clients WHERE MONTH(created_at) = MONTH(CURRENT_DATE()) AND YEAR(created_at) = YEAR(CURRENT_DATE())'),
...
//# sourceMappingURL=adminClients.js.map