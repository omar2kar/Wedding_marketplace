...
router.get('/services/:serviceId/purchase-status', auth_1.authMiddleware, async (_req, res) => {
...
// Add a review (mock)
router.post('/services/:serviceId/reviews', auth_1.authMiddleware, async (req, res) => {
    try {
        ...
        const newReview = {
            ...
            createdAt: new Date().toISOString(),
            updatedAt: new Date().toISOString(),
            clientName: req.user?.name || 'عميل جديد',
...
router.get('/vendor/reviews', vendorAuth_1.vendorAuthMiddleware, async (req, res) => {
    try {
...
router.post('/vendor/reviews/:reviewId/reply', vendorAuth_1.vendorAuthMiddleware, async (req, res) => {
    try {
...
router.put('/vendor/reviews/:reviewId/reply', vendorAuth_1.vendorAuthMiddleware, async (req, res) => {
    try {
        const { reviewId } = req.params;
        const { reply } = req.body;
        const vendorId = req.vendor?.id || 1;
...
router.delete('/vendor/reviews/:reviewId/reply', vendorAuth_1.vendorAuthMiddleware, async (req, res) => {
    try {
        const { reviewId } = req.params;
        const vendorId = req.vendor?.id || 1;
        const review = mockReviews.find(r => r.id === parseInt(reviewId));
