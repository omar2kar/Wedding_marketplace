...
async function seedReviews() {
  try {
    ...
    const existingReviews = await db.query('SELECT COUNT(*) as count FROM reviews WHERE vendor_id = 1');
    ...
    for (const review of reviews) {
      await db.query(
        `INSERT INTO reviews (client_id, vendor_id, service_id, booking_id, rating, comment, has_purchased, created_at, vendor_reply, reply_date)
         VALUES (?, ?, ?, NULL, ?, ?, ?, NOW(), ?, ?)`,
        [
          review.client_id,
          review.vendor_id,
