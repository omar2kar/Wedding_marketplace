...
const validateVendorAction = (req: any, res: any, next: any): void => {
  const { id } = req.params;
...
router.get('/admin/vendors', adminAuthMiddleware, async (req, res) => {
  try {
...
// Get pending vendors
router.get('/admin/vendors/pending', adminAuthMiddleware, async (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Failed to fetch pending vendors', details: (error as Error).message });
  }
});


// Approve vendor
router.post('/admin/vendors/:id/approve', adminAuthMiddleware, validateVendorAction, async (req, res) => {
  try {
    const { id } = req.params;
    const { notes } = req.body;
    ...
    const [vendor] = await db.query(
      'SELECT name, email, business_name FROM vendors WHERE id = ?',
      [id]
    );
    ...
    // Send approval email
    if (vendor) {
      await emailService.sendVendorApprovalEmail(vendor.email, vendor.name, vendor.business_name);
    }
...
// Reject vendor
router.post('/admin/vendors/:id/reject', adminAuthMiddleware, validateVendorAction, async (req, res) => {
  try {
    const { id } = req.params;
    const { notes } = req.body;
    const adminId = req.admin!.id;
    ...
    if (vendor) {
      await emailService.sendVendorRejectionEmail(vendor.email, vendor.name, notes || 'Application rejected');
    }
...
// Suspend vendor
router.post('/admin/vendors/:id/suspend', adminAuthMiddleware, validateVendorAction, async (req, res) => {
  try {
    const { id } = req.params;
    const { notes } = req.body;
    const adminId = req.admin!.id;
