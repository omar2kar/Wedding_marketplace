...
router.post('/admin/login', validateLoginInput, async (req, res) => {
  try {
    ...
    const admins = await db.query(
      'SELECT id, email, password, name, role, is_active FROM admins WHERE email = ? AND is_active = TRUE',
      [email]
    );
...
router.get('/admin/dashboard/stats', adminAuthMiddleware, async (req, res) => {
  try {
    ...
    const [
      ...
      db.query('SELECT COUNT(*) as count FROM vendors WHERE status = "pending"'),
      ...
      db.query('SELECT COUNT(*) as count FROM vendors WHERE DATE(created_at) = CURDATE()')
    ]);
