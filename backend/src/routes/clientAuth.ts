...
const validateLoginInput = (req: any, res: any, next: any): void => {
...
const validateRegisterInput = (req: any, res: any, next: any): void => {
...
const logLoginAttempt = async (email: string, ipAddress: string, success: boolean) => {
  try {
    await db.query(
      'INSERT INTO login_attempts (email, user_type, ip_address, success) VALUES (?, ?, ?, ?)',
      [email, 'client', ipAddress, success]
    );
  } catch (error) {
...
const generateRefreshToken = async (userId: number): Promise<string> => {
  ...
  await db.query(
    'INSERT INTO refresh_tokens (user_id, user_type, token, expires_at) VALUES (?, ?, ?, DATE_ADD(NOW(), INTERVAL 30 DAY))',
    [userId, 'client', refreshToken]
  );
...
router.post('/client/register', validateRegisterInput, async (req, res) => {
  try {
    ...
    const result = await db.query(
      `INSERT INTO clients (email, password, name, phone, role, is_verified) 
       VALUES (?, ?, ?, ?, ?, ?)`,
      [email, hashedPassword, name, phone || null, role, false]
    );
...
router.post('/client/login', validateLoginInput, async (req, res) => {
  try {
    ...
    const clients = await db.query(
      'SELECT id, email, password, name, phone, role, is_verified FROM clients WHERE email = ?',
      [email]
    );
    ...
    await Promise.all([
      db.query('UPDATE clients SET last_login_at = NOW() WHERE id = ?', [client.id]),
...
router.post('/client/refresh', async (req, res) => {
  try {
    ...
    const storedTokens = await db.query(
      'SELECT user_id FROM refresh_tokens WHERE token = ? AND user_type = ? AND expires_at > NOW()',
      [refreshToken, 'client']
    );
...
// Logout
router.post('/client/logout', authMiddleware, async (req, res) => {
  try {
    const { refreshToken } = req.body;
    ...
    if (refreshToken) {
      await db.query(
        'DELETE FROM refresh_tokens WHERE token = ? AND user_id = ? AND user_type = ?',
        [refreshToken, userId, 'client']
      );
    }
