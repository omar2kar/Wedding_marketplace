...
router.get('/admin/reports', adminAuthMiddleware, async (req, res) => {
  try {
...
router.get('/admin/reports/vendor/:id', adminAuthMiddleware, async (req, res) => {
  try {
    const { id } = req.params;
    const { startDate, endDate } = req.query;
    ...
    res.json({
      ...
      monthlyPerformance: monthlyPerformance.map((month: any) => ({
        month: month.month,
...
router.get('/admin/reports/overview', adminAuthMiddleware, async (_req, res) => {
  try {
    const [
      totalStats,
      recentActivity,
