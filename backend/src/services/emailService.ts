...
class EmailService {
  ...
  async getTemplate(templateType: string): Promise<EmailTemplate | null> {
    try {
      const [template] = await db.query(
        'SELECT subject, body FROM email_templates WHERE template_type = ? AND is_active = TRUE',
        [templateType]
      );
  ...
  async sendVendorApprovalEmail(vendorEmail: string, vendorName: string, businessName: string): Promise<boolean> {
    try {
  ...
  async sendVendorRejectionEmail(vendorEmail: string, vendorName: string, reason: string): Promise<boolean> {
    try {
  ...
  async sendServiceApprovalEmail(vendorEmail: string, vendorName: string, serviceName: string): Promise<boolean> {
    try {
